...
namespace ZeroPlus.Models.Buffers
{
    ...
    public class Buffer
    {
        ...
        public void Reserve(long capacity)
        {
            ...
            {
                long desired = Math.Max(capacity, 2 * Capacity);
                long capped = Math.Min(desired, Array.MaxLength);
                if (capacity > capped)
                    throw new InvalidOperationException($"Buffer capacity {capacity} exceeds Array.MaxLength ({Array.MaxLength}).");
                byte[] data = new byte[capped];
        ...
        /// <summary>
        /// Reserve space at the end of the buffer and return a writable span.
        /// The caller is responsible for writing into the returned span.
        /// The buffer size is advanced by the specified length.
        /// </summary>
        /// <param name="length">Number of bytes to reserve</param>
        /// <returns>A writable span at the current write position</returns>
        public Span<byte> GetAppendSpan(int length)
        {
            Reserve(_size + length);
            var span = new Span<byte>(_data, (int)_size, length);
            _size += length;
            return span;
        }
