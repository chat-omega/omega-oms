...
public class SPutDiagonal : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_DIAGONAL;
        ...
        if (legs[0]!.Security is Option option1 && legs[1]!.Security is Option option2)
        {
            if (option1.PutCall == PutCall.Put && option2.PutCall == PutCall.Put)
            {
                ...
                if (differentExpirations && differentStrikes && sameQuantity && oppositeSides)
                {
                    ...
                    string spreadType = $"PUT DIAGONAL {rootSymbols} {nearExpiration}/{farExpiration} {nearOption.Strike}/{farOption.Strike} [{spacing}]";
                    string spreadDescription = $"{(isLongDiagonal ? "LONG" : "SHORT")} {spreadType}";

                    details = new StrategyIdentification(
                        BaseType: "PUT DIAGONAL",
