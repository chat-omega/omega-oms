...
public class SPutRatioFly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_RATIO_FLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Ratio Fly requires exactly 3 legs, all put options
        ...
        string spreadType = $"RATIO FLY {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} PUT[{spacing1}]";
        string spreadDescription = $"{(isLongRatioFly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT RATIO FLY",
