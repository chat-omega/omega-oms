...
public class SReversal : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.REVERSAL;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Reversal requires exactly 3 legs
        if (legs.Count != 3)
            return false;
        ...
        // Reversal side pattern: call buy, put sell, stock sell
        bool validSidePattern = callLeg.Side == Side.Buy &&
                               putLeg.Side == Side.Sell &&
                               stockLeg.Side == Side.Sell;
        ...
        string spreadType = $"REVERSAL {stockLeg.Symbol} {expirationDate} {callOption.Strike}";
