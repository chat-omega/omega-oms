...
public class SPutRatioSpread : IStrategy
{
    ...
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        ...
        // Ratio spread requires exactly 2 legs, both put options
        if (legs.Count != 2)
            return false;
        ...
        string spreadType = $"{ratioString} RATIO PUT SPREAD {rootSymbols} {expirationDate} {lowerStrike}/{higherStrike}";
        string spreadDescription = $"{(isLongRatio ? "LONG" : "SHORT")} {spreadType}";

        // Determine appropriate baseType based on the ratio
        string baseType = "PUT RATIO";
