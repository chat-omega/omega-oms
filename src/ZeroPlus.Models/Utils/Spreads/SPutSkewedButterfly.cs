...
public class SPutSkewedButterfly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_SKEWED_BUTTERFLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Skewed Butterfly requires exactly 3 legs, all put options
        ...
        string spreadType = $"SKEWED BUTTERFLY {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} PUT[{spacing1}/{spacing2}]";
        string spreadDescription = $"{(isLongButterfly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT SKEWED BUTTERFLY",
