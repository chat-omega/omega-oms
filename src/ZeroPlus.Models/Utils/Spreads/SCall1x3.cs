...
public class SCall1x3 : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_1X3;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call 1x3 requires exactly 2 legs, all call options
        ...
        // 1x3 ratio pattern: one leg has quantity 1, other leg has quantity 3
        bool validRatioPattern = (legs[0].Quantity == 1 && legs[1].Quantity == 3) ||
                                (legs[0].Quantity == 3 && legs[1].Quantity == 1);
        ...
        string spreadType = $"CALL 1X3 {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}";
        string spreadDescription = $"{(isLongRatio ? "LONG" : "SHORT")} 1X3 RATIO CALL SPREAD {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}";
