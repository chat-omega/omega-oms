...
public class SCall2x3x1 : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_2x3x1;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call 2x3x1 requires exactly 3 legs, all call options
        ...
        // 2x3x1 quantity pattern: legs must have quantities 2, 3, 1 in some order
        ...
        // Side pattern: specific asymmetric pattern for 2x3x1
        bool validSidePattern = leg2Qty.Side == leg3Qty.Side && leg1Qty.Side != leg2Qty.Side;
        ...
        string spreadType = $"TWO THREE ONE {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} CALL[{spacing1}/{spacing2}]";
