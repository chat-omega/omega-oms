...
public class SPutButterfly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_BUTTERFLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Butterfly requires exactly 3 legs, all put options
        ...
        string spreadType = $"PUT BUTTERFLY {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} [{spacing1}]";
        string spreadDescription = $"{(isLongButterfly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT BUTTERFLY",
