...
public class SPut1x3x3x1 : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_1X3X3X1;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put 1x3x3x1 requires exactly 4 legs, all put options
        ...
        string spreadType = $"1X3X3X1 {rootSymbols} {expirationDate} {option0.Strike}/{option1.Strike}/{option2.Strike}/{option3.Strike} PUT[{spacing1}/{spacing2}/{spacing3}]";
        string spreadDescription = $"{(isLongStrategy ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT 1X3X3X1",
