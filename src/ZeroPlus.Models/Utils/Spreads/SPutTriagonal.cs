...
public class SPutTriagonal : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_TRIAGONAL;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Triagonal requires exactly 3 legs, all put options
        ...
        string spreadType = $"TRIAGONAL {rootSymbols} {nearExpiration}/{farExpiration} {option0.Strike}/{option1.Strike}/{option2.Strike} PUT";
        string spreadDescription = $"{(isLongTriagonal ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT TRIAGONAL",
