...
public class SRatioDragonfly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.RATIO_DRAGONFLY;

    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;

        // Ratio Dragonfly requires exactly 6 legs, all options
        ...
        // Must have exactly 5 distinct strikes (ratio dragonfly has middle 2 at same strike)
        ...
        // Verify quantity relationships (but NOT the specific 3-4-1 pattern of dragonfly)
        bool validQuantityRelationships =
            legs[1].Quantity == legs[4].Quantity &&  // wing1 quantities match
            legs[0].Quantity == legs[5].Quantity &&  // wing2 quantities match
            legs[2].Quantity == legs[3].Quantity;    // body quantities match

        if (!validQuantityRelationships)
            return false;

        // Must NOT be the standard dragonfly pattern (3-4-1)
        bool isStandardDragonfly =
            legs[1].Quantity == 3 &&
            legs[0].Quantity == 4 &&
            legs[2].Quantity == 1;

        if (isStandardDragonfly)
            return false;  // This is a standard dragonfly, not a ratio dragonfly

        // Build the description with ratio pattern
        int wing1Qty = legs[1].Quantity;
        int wing2Qty = legs[0].Quantity;
        int bodyQty = legs[2].Quantity;
        ...
        string spreadType = $"RATIO DRAGON FLY {rootSymbols} {expirationDate} {strikesList} RATIO[{ratioPattern}]";
