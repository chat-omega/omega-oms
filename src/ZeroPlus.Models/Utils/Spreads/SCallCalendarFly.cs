...
public class SCallCalendarFly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_CALENDAR_FLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call Calendar Fly requires exactly 3 legs, all call options
        ...
        string spreadType = $"CALENDAR FLY {rootSymbols} {nearExpiration}/{middleExpiration}/{farExpiration} {option0.Strike} CALL";
        string spreadDescription = $"{(isLongCalendarFly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "CALL CALENDAR FLY",
