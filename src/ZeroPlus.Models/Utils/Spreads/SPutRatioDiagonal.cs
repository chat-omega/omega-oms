...
public class SPutRatioDiagonal : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_CUSTOM_RATIO;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Ratio Diagonal requires exactly 4 legs, all put options
        ...
        string spreadType = $"RATIO DIAGONAL {rootSymbols} {nearExpiration.ToString("MMM-dd-yy")}/{farExpiration.ToString("MMM-dd-yy")} {strikesList} PUT RATIO[{ratioPattern}]";
        string spreadDescription = $"{(isLongStrategy ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT RATIO DIAGONAL",
