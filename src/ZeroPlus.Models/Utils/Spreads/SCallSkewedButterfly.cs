...
public class SCallSkewedButterfly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_SKEWED_BUTTERFLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call Skewed Butterfly requires exactly 3 legs, all call options
        ...
        // Skewed butterfly requires UNEQUAL spacing (this is the key difference from regular butterfly)
        bool unequalSpacing = Math.Abs(spacing1 - spacing2) >= 0.01;
        ...
        string spreadType = $"SKEWED BUTTERFLY {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} CALL[{spacing1}/{spacing2}]";
        string spreadDescription = $"{(isLongButterfly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "CALL SKEWED BUTTERFLY",
