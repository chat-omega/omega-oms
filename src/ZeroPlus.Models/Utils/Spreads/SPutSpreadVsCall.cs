...
public class SPutSpreadVsCall : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_SPREAD_VS_CALL;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Spread vs Call requires exactly 3 legs, all options
        ...
        double spacing1 = Math.Round(Math.Abs(option1.Strike - option0.Strike), 2);
        double spacing2 = Math.Round(Math.Abs(option2.Strike - option1.Strike), 2);
        ...
        string spreadType = $"PUT SPREAD VS CALL {rootSymbols} {expirationDate} {option0.Strike}/{option1.Strike}/{option2.Strike} {spacingString}";
