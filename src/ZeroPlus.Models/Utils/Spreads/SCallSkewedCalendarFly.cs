...
public class SCallSkewedCalendarFly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_SKEWED_CALENDAR_FLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call Skewed Calendar Fly requires exactly 3 legs, all call options
        ...
        // Calculate time spacing between expirations
        var option0 = (Option)leg0.Security!;
        var option1 = (Option)leg1.Security!;
        var option2 = (Option)leg2.Security!;
        
        double spacing1 = (option1.Expiration - option0.Expiration).TotalDays;
        double spacing2 = (option2.Expiration - option1.Expiration).TotalDays;
        
        // Skewed calendar fly requires UNEQUAL time spacing (key difference from regular calendar fly)
        bool unequalSpacing = Math.Abs(spacing1 - spacing2) > 1.0; // Allow 1 day tolerance for practical trading
        ...
        string spreadType = $"SKEWED CALENDAR FLY {rootSymbols} {nearExpiration}/{middleExpiration}/{farExpiration} {option0.Strike} CALL[{Math.Round(spacing1)}/{Math.Round(spacing2)}]";
        string spreadDescription = $"{(isLongCalendarFly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "CALL SKEWED CALENDAR FLY",
