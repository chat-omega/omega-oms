...
public class SPut1x2 : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_1X2;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // 1x2 requires exactly 2 legs, both put options
        if (legs.Count != 2)
            return false;
        ...
        string spreadType = $"PUT 1X2 {rootSymbols} {expirationDate} {lowerStrike}/{higherStrike}";
        string spreadDescription = $"{(isLongRatio ? "LONG" : "SHORT")} RATIO PUT SPREAD {spreadType}";
