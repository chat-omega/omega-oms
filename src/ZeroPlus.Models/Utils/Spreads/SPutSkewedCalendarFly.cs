...
public class SPutSkewedCalendarFly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.PUT_SKEWED_CALENDAR_FLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Put Skewed Calendar Fly requires exactly 3 legs, all put options
        ...
        string spreadType = $"SKEWED CALENDAR FLY {rootSymbols} {nearExpiration}/{middleExpiration}/{farExpiration} {option0.Strike} PUT[{Math.Round(spacing1)}/{Math.Round(spacing2)}]";
        string spreadDescription = $"{(isLongCalendarFly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "PUT SKEWED CALENDAR FLY",
