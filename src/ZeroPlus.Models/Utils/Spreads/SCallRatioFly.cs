...
public class SCallRatioFly : IStrategy
{
    public BaseStrategy Strategy => BaseStrategy.CALL_RATIO_FLY;
    
    public bool TryIdentify(IList<IComplexOrderLegMin> legs, out StrategyIdentification? details)
    {
        details = null;
        
        // Call Ratio Fly requires exactly 3 legs, all call options
        ...
        // Ratio Fly requires equal spacing
        bool equalSpacing = Math.Abs(spacing1 - spacing2) < 0.01;
        
        if (!equalSpacing)
            return false;
            
        // Verify the quantity pattern: outer legs (1 or 2 each), middle leg (3)
        int outerLeg1Qty = legs[0].Quantity;
        int middleLegQty = legs[1].Quantity;
        int outerLeg2Qty = legs[2].Quantity;
        
        bool validQuantityPattern = (outerLeg1Qty == 1 || outerLeg1Qty == 2) &&
                                   (outerLeg2Qty == 1 || outerLeg2Qty == 2) &&
                                   middleLegQty == 3;
        ...
        bool isLongRatioFly = spreadSide == Side.Buy;
        
        string spreadType = $"RATIO FLY {rootSymbols} {expirationDate} {option1.Strike}/{option2.Strike}/{option3.Strike} CALL[{spacing1}]";
        string spreadDescription = $"{(isLongRatioFly ? "LONG" : "SHORT")} {spreadType}";

        details = new StrategyIdentification(
            BaseType: "CALL RATIO FLY",
