...
namespace ZeroPlus.Models.Communication.Tcp
{
    ...
    public class TcpClient : IDisposable
    {
        ...
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public TcpClient(string address, int port) : this(new IPEndPoint(TryParseIp(address), port)) { }
        /// <summary>
        ...
        {
            _receiving = default;
            _receiveBuffer = new Buffers.Buffer();
            _receiveEventArg = new SocketAsyncEventArgs();

            _connectEventArg = new SocketAsyncEventArgs();

            _sending = default;
            _sendBufferMain = new Buffers.Buffer();
            _sendBufferFlush = new Buffers.Buffer();
            _sendEventArg = new SocketAsyncEventArgs();
            _sendBufferFlushOffset = default;
        ...
        /// <summary>
        /// Update TCP client address with a given server IP address and port number
        /// </summary>
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public void UpdateAddress(IPAddress address, int port)
        {
            UpdateAddress(new IPEndPoint(address, port));
        }
        /// <summary>
        /// Update TCP client address with a given server IP address and port number
        /// </summary>
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public void UpdateAddress(string address, int port)
        {
            UpdateAddress(new IPEndPoint(TryParseIp(address), port));
        }
        /// <summary>
        /// Update TCP client address with a given DNS endpoint
        /// </summary>
        /// <param name="endpoint">DNS endpoint</param>
        public void UpdateAddress(DnsEndPoint endpoint)
        {
            UpdateAddress(endpoint, endpoint.Host, endpoint.Port);
        }
        /// <summary>
        /// Update TCP client address with a given IP endpoint
        /// </summary>
        /// <param name="endpoint">IP endpoint</param>
        public void UpdateAddress(IPEndPoint endpoint)
        {
            UpdateAddress(endpoint, endpoint.Address.ToString(), endpoint.Port);
        }
        /// <summary>
        /// Update TCP client address with a given endpoint, address and port
        /// </summary>
        /// <param name="endpoint">Endpoint</param>
        /// <param name="address">Server address</param>
        /// <param name="port">Server port</param>
        private void UpdateAddress(EndPoint endpoint, string address, int port)
        {
            Address = address;
            Port = port;
            Endpoint = endpoint;
        ...
        protected static IPAddress TryParseIp(string address)
        {
            try
            {
                if (!IPAddress.TryParse(address, out var ipAddress))
                {
                    ...
                    ipAddress = hostInfo.AddressList.FirstOrDefault(x => x.AddressFamily == AddressFamily.InterNetwork);
                }
                return ipAddress!;
            }
            catch (Exception)
        ...
        public virtual bool Connect()
        {
            ...
            {
                // Call the client error handlers
                SendException(ex);
        ...
        private Buffers.Buffer _receiveBuffer;
        private SocketAsyncEventArgs _receiveEventArg;
        // Send buffer
        private readonly object _sendLock = new object();
        private bool _sending;
        private Buffers.Buffer _sendBufferMain;
        private Buffers.Buffer _sendBufferFlush;
        ...
        public virtual long Send(ReadOnlySpan<byte> buffer)
        {
            ...
            try
            {
                sent = Socket.Send(buffer, SocketFlags.None, out ec);
            }
            catch (SocketException ex)
            {
                SendException(ex);
        ...
        public void SendFramed(ReadOnlySpan<byte> message, MessagePacketType type)
        {
            const int headerSize = Message.LENGTH_OF_LENGTH_FIELD + 1;
            int packetLength = 1 + message.Length;
            int totalLength = headerSize + message.Length;
            ...
            lock (_sendLock)
            {
                ...
                if (_sendBufferMain.Size + totalLength > OptionSendBufferLimit && OptionSendBufferLimit > 0)
                {
                    SendError(SocketError.NoBufferSpaceAvailable);
                    return;
                }

                // Write the entire frame (header + body) directly into the send buffer in one operation
                Span<byte> frame = _sendBufferMain.GetAppendSpan(totalLength);
                BinaryPrimitives.WriteInt32BigEndian(frame, packetLength);
                frame[4] = (byte)type;
                message.CopyTo(frame.Slice(headerSize));
        ...
        public virtual long Receive(byte[] buffer, long offset, long size)
        {
            ...
            try
            {
                received = Socket.Receive(buffer, (int)offset, (int)size, SocketFlags.None, out ec);
            }
            catch (SocketException ex)
            {
                SendException(ex);
        ...
        private void ProcessConnect(SocketAsyncEventArgs e)
        {
            ...
            if (Socket != null && e.SocketError == SocketError.Success)
            {
                // Apply the option: keep alive
                if (OptionKeepAlive)
                {
        ...
        /// Handle error notification
        /// </summary>
        /// <param name="ex">Socket error code</param>
        protected virtual void OnSocketException(SocketException ex) { }
        ...
        private void SendException(SocketException ex)
        {
