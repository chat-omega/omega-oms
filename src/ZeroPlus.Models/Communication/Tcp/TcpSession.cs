...
namespace ZeroPlus.Models.Communication.Tcp
{
    ...
    public class TcpSession : IDisposable
    {
        ...
        /// </summary>
        public int OptionSendBufferLimit { get; set; } = int.MaxValue - 1024;
        /// <summary>
        ...
        /// </summary>
        /// <param name="buffer">Buffer to send as a span of bytes</param>
        /// <param name="ignoreBufferCheck">Buffer to send as a span of bytes</param>
        /// <returns>'true' if the data was successfully sent, 'false' if the session is not connected</returns>
        public virtual bool SendAsync(ReadOnlySpan<byte> buffer, bool ignoreBufferCheck)
        {
            if (!IsConnected)
            {
                return false;
            ...
            lock (_sendLock)
            {
                ...
                if (_sendBufferMain.Size + buffer.Length > OptionSendBufferLimit && OptionSendBufferLimit > 0 && !ignoreBufferCheck)
                {
                    SendError(SocketError.NoBufferSpaceAvailable);
                    return false;
                }
        ...
        private void TryReceive()
        {
            ...
            while (process)
            {
                ...
                try
                {
                    ...
                    if (Socket != null && !Socket.ReceiveAsync(_receiveEventArg))
                    {
        ...
        private void TrySend()
        {
            ...
            while (process)
            {
                ...
                try
                {
                    ...
                    if (Socket != null && !Socket.SendAsync(_sendEventArg))
                    {
        ...
        /// </summary>
        protected virtual void OnSessionConnected() { }
        /// <summary>
