...
{
    public class IronCondorSpreadsGenerator : SpreadGenerator
    {
        public IronCondorSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

        public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, List<Option> leg4Options, IIronCondorSpreadsGeneratorSettings ironCondorSpreadsSettings, int count, CancellationToken token, bool isSample = false)
        {
            List<Option> putOptionsChain = leg1Options.Union(leg2Options).Distinct().ToList();
            List<Option> callOptionsChain = leg3Options.Union(leg4Options).Distinct().ToList();
            Option? putOption = putOptionsChain.FirstOrDefault();
            Option? callOption = callOptionsChain.FirstOrDefault();
            SpreadGeneratorResults spreadGeneratorResults = new(putOption?.Underlying?.Symbol, putOption?.PutCall, Strategy.IronCondor);
            ...
            _logger.LogInformation($"[Start] {nameof(IronCondorSpreadsGenerator)}. " +
            ...
            if (!isSample)
            {
                RequestData(putOptionsChain, callOptionsChain, ironCondorSpreadsSettings);

                leg1Options = await ApplyLeg1Filters(leg1Options, ironCondorSpreadsSettings, spreadGeneratorResults, putOptionsChain, token);
                leg2Options = await ApplyLeg2Filters(leg2Options, ironCondorSpreadsSettings, spreadGeneratorResults, putOptionsChain, token);
                leg3Options = await ApplyLeg3Filters(leg3Options, ironCondorSpreadsSettings, spreadGeneratorResults, callOptionsChain, token);
                leg4Options = await ApplyLeg4Filters(leg4Options, ironCondorSpreadsSettings, spreadGeneratorResults, callOptionsChain, token);
            }
            ...
            List<List<SpreadHolder>> results = GenerateSpreads(leg1OptionsGroup, leg2OptionsGroup, leg3OptionsGroup, leg4OptionsGroup, ironCondorSpreadsSettings, token, isSample);
            ...
            if (!isSample)
            {
                ...
                {
                    validResults.Add(ApplySpreadFilters(ironCondorSpreadsSettings, spreadGeneratorResults, groupResult, callOptionsChain, token));
                }
            ...
            _logger.LogInformation($"[Finish] {nameof(IronCondorSpreadsGenerator)}. " +
                      $"For {putOption.Underlying?.Symbol}, " +
        ...
        private void RequestData(List<Option> putOptionsChain, List<Option> callOptionsChain, IIronCondorSpreadsGeneratorSettings settings)
        {
        ...
        private Task<List<Option>> ApplyLeg1Filters(List<Option> putOptionsChain, IIronCondorSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(putOptionsChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg2Filters(List<Option> putOptionsChain, IIronCondorSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(putOptionsChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg3Filters(List<Option> callOptionsChain, IIronCondorSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(callOptionsChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg4Filters(List<Option> callOptionsChain, IIronCondorSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(callOptionsChain, new LegFilterConfig(
        ...
        private async Task<List<SpreadHolder>> ApplySpreadFilters(IIronCondorSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
        {
        ...
        private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
                                                      ...
                                                      IIronCondorSpreadsGeneratorSettings settings,
            ...
            foreach (List<Option> leg1Options in leg1OptionsGroup)
            {
                ...
                for (int leg1Index = 0; leg1Index < leg1Options.Count; leg1Index++)
                {
                    ...
                    if (spacing1List != null)
                    {
                        ...
                        {
                            token.ThrowIfCancellationRequested();
                            Option? leg2WithCurrentSpacing = null;

                            leg2WithCurrentSpacing = leg2Options.FirstOrDefault(x => x.Strike == leg1.Strike + spacing);
                    ...
                    else
                    {
                        ...
                        leg2 = leg2Options.FirstOrDefault(x => x.Strike > leg1.Strike);
                    ...
                    foreach (int leg2Index in leg2Indexes)
                    {
                        ...
                        if (spacing2List != null)
                        {
                            ...
                            {
                                token.ThrowIfCancellationRequested();
                                Option? leg3WithCurrentSpacing = null;

                                leg3WithCurrentSpacing = leg3Options.FirstOrDefault(x => x.Strike == leg2.Strike + spacing);
                        ...
                        else
                        {
                            ...
                            leg3 = leg3Options.FirstOrDefault(x => x.Strike > leg2.Strike);
                        ...
                        foreach (int leg3Index in leg3Indexes)
                        {
                            ...
                            if (spacing3List != null)
                            {
                                ...
                                {
                                    token.ThrowIfCancellationRequested();
                                    Option? leg4WithCurrentSpacing = null;

                                    leg4WithCurrentSpacing = leg4Options.FirstOrDefault(x => x.Strike == leg3.Strike + spacing);
                            ...
                            else
                            {
                                ...
                                leg4 = leg4Options.FirstOrDefault(x => x.Strike > leg3.Strike);
