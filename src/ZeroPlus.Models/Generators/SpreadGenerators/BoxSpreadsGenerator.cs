...
public class BoxSpreadsGenerator : SpreadGenerator
{
    public BoxSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore) : 
        base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
    {
    }

    public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, List<Option> leg4Options, IBoxSpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
    {
        ...
        SpreadGeneratorResults spreadGeneratorResults = new(putOption?.Underlying?.Symbol, putOption?.PutCall, Strategy.Box);
        ...
        _logger.LogInformation($"[Start] {nameof(BoxSpreadsGenerator)}. " +
        ...
        _logger.LogInformation($"[Finish] {nameof(BoxSpreadsGenerator)}. " +
                               $"For {putOption.Underlying?.Symbol} " +
    ...
    private void RequestData(List<Option> putOptionsChain, List<Option> callOptionsChain, IBoxSpreadsGeneratorSettings settings)
    {
    ...
    private Task<List<Option>> ApplyLeg1Filters(List<Option> putOptionsChain, IBoxSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
    ...
    private Task<List<Option>> ApplyLeg2Filters(List<Option> putOptionsChain, IBoxSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
        => ApplyLegFilters(putOptionsChain, new LegFilterConfig(
    ...
    private Task<List<Option>> ApplyLeg3Filters(List<Option> callOptionsChain, IBoxSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
    ...
    private Task<List<Option>> ApplyLeg4Filters(List<Option> callOptionsChain, IBoxSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
        => ApplyLegFilters(callOptionsChain, new LegFilterConfig(
    ...
    private async Task<List<SpreadHolder>> ApplySpreadFilters(IBoxSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
    {
    ...
    private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
        ...
        IBoxSpreadsGeneratorSettings settings,
        ...
        foreach (List<Option> leg1Options in leg1OptionsGroup)
        {
            ...
            for (int leg1Index = 0; leg1Index < leg1Options.Count; leg1Index++)
            {
                ...
                foreach (int leg2Index in leg2Indexes)
                {
                    ...
                    Option? leg3 = leg3Group.FirstOrDefault(x => x.Strike == leg1.Strike);
                    ...
                    Option? leg4 = leg3Group.FirstOrDefault(x => x.Strike == leg2.Strike);
