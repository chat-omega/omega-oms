...
{
    public class SkewedButterflySpreadsGenerator : SpreadGenerator
    {
        public SkewedButterflySpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

        public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, ISkewedButterflySpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
        {
            ...
            SpreadGeneratorResults spreadGeneratorResults = new(option?.Underlying?.Symbol, option?.PutCall, Strategy.SkewedButterfly);
            if (option == null)
            {
            ...
            _logger.LogInformation($"[Start] {nameof(SkewedButterflySpreadsGenerator)}. " +
            ...
            _logger.LogInformation($"[Finish] {nameof(SkewedButterflySpreadsGenerator)}. " +
                      $"For {option.Underlying?.Symbol} {type}, " +
                      $"using {optionChain.Count} options, " +
        ...
        private void RequestData(List<Option> optionChain, ISkewedButterflySpreadsGeneratorSettings settings)
        {
        ...
        private async Task<List<Option>> ApplyLeg1Filters(List<Option> optionChain, ISkewedButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
        {
        ...
        private async Task<List<Option>> ApplyLeg2Filters(List<Option> optionChain, ISkewedButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
        {
        ...
        private async Task<List<Option>> ApplyLeg3Filters(List<Option> optionChain, ISkewedButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
        {
        ...
        private async Task<List<SpreadHolder>> ApplySpreadFilters(ISkewedButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
        {
        ...
        private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
                                                      ...
                                                      PutCall type,
                                                      ISkewedButterflySpreadsGeneratorSettings settings,
                        ...
                        else
                        {
                            ...
                            if (range2Enabled)
                            {
                                Option? leg3Start = leg3Group.Skip(leg3StartIndex)
                                                         .FirstOrDefault(x => Math.Abs(leg2.Strike - x.Strike) >= range2Floor);
                                Option? leg3End = leg3Group.Skip(leg3StartIndex)
                                                       .LastOrDefault(x => Math.Abs(leg2.Strike - x.Strike) <= range2Ceil);
