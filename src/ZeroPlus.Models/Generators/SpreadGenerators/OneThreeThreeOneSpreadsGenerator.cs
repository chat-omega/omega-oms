...
{
    public class OneThreeThreeOneSpreadsGenerator : SpreadGenerator
    {
        public OneThreeThreeOneSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

        public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, List<Option> leg4Options, IOneThreeThreeOneSpreadsGeneratorSettings OneThreeThreeOneSpreadsSettings, int count, CancellationToken token, bool isSample = false)
        {
            ...
            SpreadGeneratorResults spreadGeneratorResults = new(option?.Underlying?.Symbol, option?.PutCall, Strategy.OneThreeThreeOne);
            if (option == null)
            {
            ...
            _logger.LogInformation($"[Start] {nameof(OneThreeThreeOneSpreadsGenerator)}. " +
            ...
            if (!isSample)
            {
                RequestData(optionChain, OneThreeThreeOneSpreadsSettings);

                leg1Options = await ApplyLeg1Filters(leg1Options, OneThreeThreeOneSpreadsSettings, spreadGeneratorResults, token);
                leg2Options = await ApplyLeg2Filters(leg2Options, OneThreeThreeOneSpreadsSettings, spreadGeneratorResults, token);
                leg3Options = await ApplyLeg3Filters(leg3Options, OneThreeThreeOneSpreadsSettings, spreadGeneratorResults, token);
                leg4Options = await ApplyLeg4Filters(leg4Options, OneThreeThreeOneSpreadsSettings, spreadGeneratorResults, token);
            }
            ...
            List<List<SpreadHolder>> results = GenerateSpreads(leg1OptionsGroup, leg2OptionsGroup, leg3OptionsGroup, leg4OptionsGroup, type, OneThreeThreeOneSpreadsSettings, token, isSample);
            ...
            if (!isSample)
            {
                ...
                {
                    validResults.Add(ApplySpreadFilters(OneThreeThreeOneSpreadsSettings, spreadGeneratorResults, groupResult, optionChain, token));
                }
            ...
            foreach (List<SpreadHolder> result in results)
            {
                ...
                foreach (SpreadHolder spread in groupResult)
                {
                    ...
                    spreadModel.Legs.Add(new SpreadLeg(leg3, Side.Buy, 3));
                    spreadModel.Legs.Add(new SpreadLeg(leg4, Side.Sell));
            ...
            _logger.LogInformation($"[Finish] {nameof(OneThreeThreeOneSpreadsGenerator)}. " +
                      $"For {option.Underlying?.Symbol} {type}, " +
                      $"using {optionChain.Count} options, " +
        ...
        private void RequestData(List<Option> optionChain, IOneThreeThreeOneSpreadsGeneratorSettings settings)
        {
        ...
        private Task<List<Option>> ApplyLeg1Filters(List<Option> optionChain, IOneThreeThreeOneSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
        ...
        private Task<List<Option>> ApplyLeg2Filters(List<Option> optionChain, IOneThreeThreeOneSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
            => ApplyLegFilters(optionChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg3Filters(List<Option> optionChain, IOneThreeThreeOneSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
            => ApplyLegFilters(optionChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg4Filters(List<Option> optionChain, IOneThreeThreeOneSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
            => ApplyLegFilters(optionChain, new LegFilterConfig(
        ...
        private async Task<List<SpreadHolder>> ApplySpreadFilters(IOneThreeThreeOneSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
        {
            if (settings.SpreadDeltaRangeEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    double spreadDelta = Math.Abs(leg1Delta - 3 * leg2Delta + 3 * leg3Delta - leg4Delta);
            ...
            if (settings.SpreadTheoRangeEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    double spreadTheo = leg1Theo - 3 * leg2Theo + 3 * leg3Theo - leg4Theo;
            ...
            if (settings.SpreadVegaRangeEnabled && results.Count > 0)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    double spreadVega = leg1Vega - 3 * leg2Vega + 3 * leg3Vega - leg4Vega;
            ...
            if (settings.SpreadVolaToHanweckDiffEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    double spreadTheo = leg1Theo - 3 * leg2Theo + 3 * leg3Theo - leg4Theo;
                    double spreadVola = leg1Vola - 3 * leg2Vola + 3 * leg3Vola - leg4Vola;
                ...
                settings.SpreadTheoAbsMidEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    // For buys  + _bidStore
                    // For sells - _askStore
                    double low = leg1Bid - 3 * leg2Ask + 3 * leg3Ask - leg4Bid;

                    // For buys  + _askStore
                    // For sells - _bidStore
                    double high = leg1Ask - 3 * leg2Bid + 3 * leg3Bid - leg4Ask;
                    ...
                    if (settings.SpreadEmaToMidRangeEnabled)
                    {
                        ...
                        double spreadEma = leg1Ema - 3 * leg2Ema + 3 * leg3Ema - leg4Ema;
                        double spreadMid = (low + high) / 2;
        ...
        private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
                                                     ...
                                                     PutCall type,
                                                     IOneThreeThreeOneSpreadsGeneratorSettings settings,
            ...
            foreach (List<Option> leg1Options in leg1OptionsGroup)
            {
                ...
                for (int leg1Index = 0; leg1Index < leg1Options.Count; leg1Index++)
                {
                    ...
                    if (spacing1List != null)
                    {
                        ...
                        {
                            token.ThrowIfCancellationRequested();
                            Option? leg2WithCurrentSpacing = leg2Options.FirstOrDefault(x => x.Strike == (isCall ? leg1.Strike + spacing : leg1.Strike - spacing));
                    ...
                    else
                    {
                        ...
                        if (range1Enabled)
                        {
                            Option? leg2Start = leg2Options.Skip(leg2StartIndex)
                                                       .FirstOrDefault(x => Math.Abs(leg1.Strike - x.Strike) >= range1Floor);
                            Option? leg2End = leg2Options.Skip(leg2StartIndex)
                                                     .LastOrDefault(x => Math.Abs(leg1.Strike - x.Strike) <= range1Ceil);
                    ...
                    foreach (int leg2Index in leg2Indexes)
                    {
                        ...
                        double spacing = leg2.Strike - leg1.Strike;
                        double leg3Strike = leg2.Strike + spacing;

                        Option? leg3 = leg3Group.FirstOrDefault(x => x.Strike == leg3Strike);
                        ...
                        double leg4Strike = leg3.Strike + spacing;
