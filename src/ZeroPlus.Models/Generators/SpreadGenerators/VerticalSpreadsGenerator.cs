...
{
    public class VerticalSpreadsGenerator : SpreadGenerator
    {
        public VerticalSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

        public async Task<SpreadGeneratorStat> CalculateStatsAsync(List<Option> optionChain, IVerticalSpreadsGeneratorSettings settings, CancellationToken token)
        {
            ...
            List<Option>? leg1Options = await ApplyLeg1Filters(optionChain, settings, new SpreadGeneratorResults(option.Underlying?.Symbol, option.PutCall, Strategy.Vertical), token);
            List<Option>? leg2Options = await ApplyLeg2Filters(optionChain, settings, new SpreadGeneratorResults(option.Underlying?.Symbol, option.PutCall, Strategy.Vertical), token);
            ...
            foreach (IGrouping<DateTime, Option> leg2Exp in leg2Options.GroupBy(x => x.Expiration)
                                               .Where(x => x.Count() > 0))
            {
                string title = leg2Exp.Key.ToString("MMM dd yy");
                if (spreadGeneratorResults.Details.Any(x => x.Title == title))
                {
                    spreadGeneratorResults.Details.First(x => x.Title == title).Leg2Count = leg2Exp.Count();
                }
                else
                {
                    spreadGeneratorResults.Details.Add(new SpreadGeneratorStat()
                    {
                        Title = leg2Exp.Key.ToString("MMM dd yy"),
                        Leg2Count = leg2Exp.Count(),
                    });
                }
            }

            spreadGeneratorResults.Leg1Count = spreadGeneratorResults.Details.Sum(x => x.Leg1Count);
            spreadGeneratorResults.Leg2Count = spreadGeneratorResults.Details.Sum(x => x.Leg2Count);

            return spreadGeneratorResults;
        }

        public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, IVerticalSpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
        {
            List<Option> optionChain = leg1Options.Union(leg2Options).Distinct().ToList();
            Option? option = optionChain.FirstOrDefault();
            SpreadGeneratorResults spreadGeneratorResults = new(option?.Underlying?.Symbol, option?.PutCall, Strategy.Vertical);
            if (option == null)
            {
                spreadGeneratorResults.Errors.Add("Option chain is empty.");
                return spreadGeneratorResults;
            ...
            _logger.LogInformation($"[Start] {nameof(VerticalSpreadsGenerator)}. " +
            ...
            _logger.LogInformation($"[Finish] {nameof(VerticalSpreadsGenerator)}. " +
                      $"For {option.Underlying?.Symbol} {type}, " +
                      $"using {optionChain.Count} options, " +
        ...
        private void RequestData(List<Option> optionChain, IVerticalSpreadsGeneratorSettings settings)
        {
        ...
        private Task<List<Option>> ApplyLeg1Filters(List<Option> optionChain, IVerticalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
        ...
        private Task<List<Option>> ApplyLeg2Filters(List<Option> optionChain, IVerticalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
            => ApplyLegFilters(optionChain, new LegFilterConfig(
        ...
        private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
                                                     ...
                                                     PutCall type,
                                                     IVerticalSpreadsGeneratorSettings settings,
        ...
        private async Task<List<SpreadHolder>> ApplySpreadFilters(IVerticalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
        {
            ...
            if (settings.SpreadTheoRangeEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    double spreadTheo = Math.Abs(leg1Theo - leg2Theo);
                ...
                settings.SpreadTheoAbsMidEnabled)
            {
                ...
                foreach (SpreadHolder spread in results)
                {
                    ...
                    if (settings.SpreadEmaToMidRangeEnabled)
                    {
                        ...
                        double spreadEma = Math.Abs(leg1Ema - leg2Ema);
                        double spreadMid = (low + high) / 2;
