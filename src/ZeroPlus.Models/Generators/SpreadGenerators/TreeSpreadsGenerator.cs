...
public class TreeSpreadsGenerator : SpreadGenerator
{
    public TreeSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
        : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
    {
    }

    public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, ITreeSpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
    {
        ...
        SpreadGeneratorResults spreadGeneratorResults = new(option?.Underlying?.Symbol, option?.PutCall, Strategy.Tree);
        if (option == null)
        {
        ...
        _logger.LogInformation($"[Start] {nameof(TreeSpreadsGenerator)}. " +
        ...
        foreach (List<SpreadHolder> result in results)
        {
            ...
            foreach (SpreadHolder spread in groupResult)
            {
                ...
                string tos = leg1.Symbol + "-" + leg2.Symbol + "-" + leg3.Symbol;
        ...
        _logger.LogInformation($"[Finish] {nameof(TreeSpreadsGenerator)}. " +
                               $"For {option.Underlying?.Symbol} {type}, " +
                               $"using {optionChain.Count} options, " +
    ...
    private void RequestData(List<Option> optionChain, ITreeSpreadsGeneratorSettings settings)
    {
    ...
    private async Task<List<Option>> ApplyLeg1Filters(List<Option> optionChain, ITreeSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
    {
    ...
    private async Task<List<Option>> ApplyLeg2Filters(List<Option> optionChain, ITreeSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
    {
    ...
    private async Task<List<Option>> ApplyLeg3Filters(List<Option> optionChain, ITreeSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
    {
    ...
    private async Task<List<SpreadHolder>> ApplySpreadFilters(ITreeSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
    {
        if (settings.SpreadDeltaRangeEnabled)
        {
            ...
            foreach (SpreadHolder spread in results)
            {
                ...
                double spreadDelta = Math.Abs(leg1Delta - leg2Delta - leg3Delta);
        ...
        if (settings.SpreadTheoRangeEnabled)
        {
            ...
            foreach (SpreadHolder spread in results)
            {
                ...
                double spreadTheo = leg1Theo - leg2Theo - leg3Theo;
        ...
        if (settings.SpreadVegaRangeEnabled && results.Count > 0)
        {
            ...
            foreach (SpreadHolder spread in results)
            {
                ...
                double spreadVega = leg1Vega - leg2Vega - leg3Vega;
        ...
        if (settings.SpreadVolaToHanweckDiffEnabled)
        {
            ...
            foreach (SpreadHolder spread in results)
            {
                ...
                double spreadTheo = leg1Theo - leg2Theo - leg3Theo;
                double spreadVola = leg1Vola - leg2Vola - leg3Vola;
			...
			settings.SpreadTheoAbsMidEnabled)
        {
            ...
            foreach (SpreadHolder spread in results)
            {
                ...
                // For buys  + _bidStore
                // For sells - _askStore
                double low = leg1Bid - leg2Ask - leg3Bid;

                // For buys  + _askStore
                // For sells - _bidStore
                double high = leg1Ask - leg2Bid - leg3Ask;
                ...
                if (settings.SpreadEmaToMidRangeEnabled)
                {
                    ...
                    double spreadEma = leg1Ema - leg2Ema - leg3Ema;
                    double spreadMid = (low + high) / 2;
    ...
    private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
        ...
        PutCall type,
        ITreeSpreadsGeneratorSettings settings,
        ...
        foreach (List<Option> leg1Options in leg1OptionsGroup)
        {
            ...
            for (int leg1Index = 0; leg1Index < leg1Options.Count; leg1Index++)
            {
                ...
                foreach (int leg2Index in leg2Indexes)
                {
                    ...
                    leg3ByExpiration.TryGetValue(leg2.Expiration, out List<Option>? leg3Group);
                    ...
                    if (spacing2List != null)
                    {
                        foreach (double spacing in spacing2List)
                        {
                            ...
                            if (!leg3StrikeToIndex[leg2.Expiration].TryGetValue(targetStrike, out int leg3Idx))
                            {
                                continue;
                            }

                            leg3Indexes.Add(leg3Idx);
                        }
                    }
                    else
                    {
                        Option? leg3 = isCall
                            ? leg3Group.FirstOrDefault(x => x.Strike > leg2.Strike)
                            : leg3Group.FirstOrDefault(x => x.Strike < leg2.Strike);
                        ...
                        int leg3StartIndex = leg3StrikeToIndex[leg2.Expiration][leg3.Strike];
                        int leg3EndIndex = leg3Group.Count;

                        if (spacing2RangeEnabled)
                        {
                            Option? leg3Start = leg3Group.Skip(leg3StartIndex)
                                .FirstOrDefault(x => Math.Abs(leg2.Strike - x.Strike) >= spacing2RangeFloor);
                            Option? leg3End = leg3Group.Skip(leg3StartIndex)
                            ...
                            {
                                leg3StartIndex = leg3StrikeToIndex[leg2.Expiration][leg3Start.Strike];
                                leg3EndIndex = leg3StrikeToIndex[leg2.Expiration][leg3End.Strike] + 1;
                            }
                        }

                        for (int i = leg3StartIndex; i < leg3EndIndex && i < leg3Group.Count; i++)
                        {
                            token.ThrowIfCancellationRequested();
                            leg3Indexes.Add(i);
                        }
                    ...
                    {
                        token.ThrowIfCancellationRequested();
                        Option? leg3 = leg3Group[leg3Index];

                        if (Math.Abs(leg1.Strike - leg2.Strike) == Math.Abs(leg2.Strike - leg3.Strike))
                        {
                            continue;
                        }
