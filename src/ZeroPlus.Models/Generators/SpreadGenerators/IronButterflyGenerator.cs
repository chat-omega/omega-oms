...
{
    public class IronButterflySpreadsGenerator : SpreadGenerator
    {
        public IronButterflySpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

        public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, List<Option> leg3Options, List<Option> leg4Options, IIronButterflySpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
        {
            ...
            SpreadGeneratorResults spreadGeneratorResults = new(putOption?.Underlying?.Symbol, putOption?.PutCall, Strategy.IronButterfly);
            ...
            _logger.LogInformation($"[Start] {nameof(IronButterflySpreadsGenerator)}. " +
            ...
            if (!isSample)
            {
                RequestData(putOptionsChain, callOptionsChain, settings);

                leg1Options = await ApplyLeg1Filters(leg1Options, settings, spreadGeneratorResults, putOptionsChain, token);
                leg2Options = await ApplyLeg2Filters(leg2Options, settings, spreadGeneratorResults, putOptionsChain, token);
                leg3Options = await ApplyLeg3Filters(leg3Options, settings, spreadGeneratorResults, callOptionsChain, token);
                leg4Options = await ApplyLeg4Filters(leg4Options, settings, spreadGeneratorResults, callOptionsChain, token);
            }
            ...
            _logger.LogInformation($"[Finish] {nameof(IronButterflySpreadsGenerator)}. " +
                      $"For {putOption.Underlying?.Symbol} " +
        ...
        private void RequestData(List<Option> putOptionsChain, List<Option> callOptionsChain, IIronButterflySpreadsGeneratorSettings settings)
        {
        ...
        private Task<List<Option>> ApplyLeg1Filters(List<Option> putOptionsChain, IIronButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
        ...
        private Task<List<Option>> ApplyLeg2Filters(List<Option> putOptionsChain, IIronButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(putOptionsChain, new LegFilterConfig(
        ...
        private Task<List<Option>> ApplyLeg3Filters(List<Option> callOptionsChain, IIronButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
        ...
        private Task<List<Option>> ApplyLeg4Filters(List<Option> callOptionsChain, IIronButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<Option> optionChain, CancellationToken token)
            => ApplyLegFilters(callOptionsChain, new LegFilterConfig(
        ...
        private async Task<List<SpreadHolder>> ApplySpreadFilters(IIronButterflySpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, List<Option> optionChain, CancellationToken token)
        {
        ...
        private List<List<SpreadHolder>> GenerateSpreads(List<List<Option>> leg1OptionsGroup,
                                                      ...
                                                      IIronButterflySpreadsGeneratorSettings settings,
