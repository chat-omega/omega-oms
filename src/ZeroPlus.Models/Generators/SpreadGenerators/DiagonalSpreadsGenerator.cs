...
{
	public class DiagonalSpreadsGenerator : SpreadGenerator
	{
		private const double STRIKE_TOLERANCE = .01;

        public DiagonalSpreadsGenerator(ILogger logger, IDataStore deltaStore, IDataStore theoStore, IDataStore vegaStore, IDataStore bidStore, IDataStore askStore, IDataStore lastStore, IDataStore emaStore, IDataStore wVegaStore, IDataStore volaStore)
            : base(logger, deltaStore, theoStore, vegaStore, bidStore, askStore, lastStore, emaStore, wVegaStore, volaStore)
        {
        }

		public async Task<SpreadGeneratorResults> GenerateAsync(List<Option> leg1Options, List<Option> leg2Options, IDiagonalSpreadsGeneratorSettings settings, int count, CancellationToken token, bool isSample = false)
		{
			List<Option> optionChain = leg1Options.Union(leg2Options).Distinct().ToList();
			Option? option = optionChain.FirstOrDefault();
			SpreadGeneratorResults spreadGeneratorResults = new(option?.Underlying?.Symbol, option?.PutCall, Strategy.Diagonal);
			if (option == null)
			{
			...
			_logger.LogInformation($"[Start] {nameof(DiagonalSpreadsGenerator)}. " +
			...
			foreach (List<SpreadHolder> group in GenerateSpreadsStreaming(leg1OptionsGroup, leg2OptionsGroup, expirationDates, type, settings, token, isSample))
			{
				List<SpreadHolder> filtered = !isSample
					? await ApplySpreadFilters(settings, spreadGeneratorResults, group, token)
					: group;
			...
			int leg1Ratio = settings.Leg1Ratio;
			int leg2Ratio = settings.Leg2Ratio;
			string leg1RatioPrefix = leg1Ratio > 1 ? leg1Ratio + "*" : "";
			string leg2RatioPrefix = leg2Ratio > 1 ? leg2Ratio + "*" : "";

			foreach (List<SpreadHolder> result in results)
			{
				token.ThrowIfCancellationRequested();

				foreach (SpreadHolder spread in result)
				{
					token.ThrowIfCancellationRequested();
					...
					{
						Spread spreadModel = new("+" + leg1RatioPrefix + leg1.Symbol + "-" + leg2RatioPrefix + leg2.Symbol, spread.Width);
						spreadModel.Legs.Add(new SpreadLeg(leg1, Side.Buy, leg1Ratio));
						spreadModel.Legs.Add(new SpreadLeg(leg2, Side.Sell, leg2Ratio));
						spreadGeneratorResults.Spreads.Add(spreadModel);
					}
					else
					{
						Spread spreadModel = new(leg2RatioPrefix + leg2.Symbol + "-" + leg1RatioPrefix + leg1.Symbol, spread.Width);
						spreadModel.Legs.Add(new SpreadLeg(leg2, Side.Buy, leg2Ratio));
						spreadModel.Legs.Add(new SpreadLeg(leg1, Side.Sell, leg1Ratio));
						spreadGeneratorResults.Spreads.Add(spreadModel);
					}
				}
			}
			...
			_logger.LogInformation($"[Finish] {nameof(DiagonalSpreadsGenerator)}. " +
					  $"For {option.Underlying?.Symbol} {type}, " +
					  $"using {optionChain.Count} options, " +
		...
		private void RequestData(List<Option> optionChain, IDiagonalSpreadsGeneratorSettings settings)
		{
		...
		private Task<List<Option>> ApplyLeg1Filters(List<Option> optionChain, IDiagonalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
		...
		private Task<List<Option>> ApplyLeg2Filters(List<Option> optionChain, IDiagonalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, CancellationToken token)
			=> ApplyLegFilters(optionChain, new LegFilterConfig(
		...
		private async Task<List<SpreadHolder>> ApplySpreadFilters(IDiagonalSpreadsGeneratorSettings settings, SpreadGeneratorResults spreadGeneratorResults, List<SpreadHolder> results, CancellationToken token)
		{
				...
				settings.SpreadTheoAbsMidEnabled)
			{
				...
				double leg2Ask = await _askStore.GetDataAsync(leg2.Symbol);
					...
					// For buys  + _bidStore
					// For sells - _askStore
					double low = -settings.Leg1Ratio * leg1Bid + settings.Leg2Ratio * leg2Ask;

					// For buys  + _askStore
					// For sells - _bidStore
					double high = -settings.Leg1Ratio * leg1Ask + settings.Leg2Ratio * leg2Bid;
		...
		private IEnumerable<List<SpreadHolder>> GenerateSpreadsStreaming(List<List<Option>> leg1OptionsGroup,
													  List<List<Option>> leg2OptionsGroup,
													  List<DateTime> expirationDates,
													  PutCall type,
													  IDiagonalSpreadsGeneratorSettings settings,
													  ...
													  bool isSample)
		{
			...
			List<double>? strikeSpacingList = settings.SpreadStrikeSpacingListEnabled
				? settings.SpreadStrikeSpacingList!.Split(',')
			...
			bool strikeRangeEnabled = settings.SpreadStrikeSpacingRangeEnabled;
			double strikeRangeFloor = settings.SpreadStrikeSpacingRangeFloor;
			double strikeRangeCeil = settings.SpreadStrikeSpacingRangeCeil;
			...
			bool needsGapCheck = gapList != null || gapRangeEnabled;
			DateTime[] sortedExpDates = null!;
			Dictionary<DateTime, int> expDateToSortedIndex = null!;
			if (needsGapCheck)
			{
				sortedExpDates = expirationDates.Distinct().OrderBy(x => x).ToArray();
				expDateToSortedIndex = new Dictionary<DateTime, int>(sortedExpDates.Length);
				for (int i = 0; i < sortedExpDates.Length; i++)
					expDateToSortedIndex[sortedExpDates[i]] = i;
			}

			Dictionary<DateTime, int> leg2DateToIndex = new(leg2OptionsGroup.Count);
			for (int i = 0; i < leg2OptionsGroup.Count; i++)
				if (leg2OptionsGroup[i].Count > 0)
					leg2DateToIndex[leg2OptionsGroup[i][0].Expiration.Date] = i;

			List<int> leg2ExpirationIndexes = new();
			List<int> leg2Indexes = new();

			for (int expirationIndex = 0; expirationIndex < leg1OptionsGroup.Count; expirationIndex++)
			{
				token.ThrowIfCancellationRequested();
				List<SpreadHolder> groupResults = new();
				List<Option> leg1Options = leg1OptionsGroup[expirationIndex];
				for (int leg1Index = 0; leg1Index < leg1Options.Count; leg1Index++)
				{
					token.ThrowIfCancellationRequested();
					Option? leg1 = leg1Options[leg1Index];

					leg2ExpirationIndexes.Clear();
					...
					if (spacingList != null)
					{
						...
						{
							token.ThrowIfCancellationRequested();
							if (!leg2DateToIndex.TryGetValue((leg1.Expiration + TimeSpan.FromDays(spacing)).Date, out int groupIdx))
							{
								continue;
							}

							leg2ExpirationIndexes.Add(groupIdx);
						}
					}
					else
					{
						if (leg2OptionsGroup.Count == 0)
						{
							continue;
						}
						...
						if (rangeEnabled)
						{
							...
							{
								token.ThrowIfCancellationRequested();
								double spacing = Math.Abs((leg1.Expiration - leg2OptionsGroup[i][0].Expiration).TotalDays);
								if (spacing >= rangeFloor && spacing <= rangeCeil)
								{
									leg2ExpirationIndexes.Add(i);
								}
							}
						}
						else
					...
					for (int expIdx = 0; expIdx < leg2ExpirationIndexes.Count; expIdx++)
					{
						int leg2ExpIndex = leg2ExpirationIndexes[expIdx];
						token.ThrowIfCancellationRequested();
						List<Option> optionsInGroup = leg2OptionsGroup[leg2ExpIndex];
						if (optionsInGroup.Count == 0) continue;

						leg2Indexes.Clear();

						if (strikeSpacingList != null)
						{
							foreach (double spacing in strikeSpacingList)
							{
								token.ThrowIfCancellationRequested();

								for (int si = 0; si < optionsInGroup.Count; si++)
								{
									if (Math.Abs(optionsInGroup[si].Strike - leg1.Strike) <= spacing)
									{
										leg2Indexes.Add(si);
										break;
									}
						...
						{
							if (strikeRangeEnabled)
							{
								for (int i = 0; i < optionsInGroup.Count; i++)
								{
									token.ThrowIfCancellationRequested();
									var spacing = Math.Abs(leg1.Strike - optionsInGroup[i].Strike);
									if (spacing >= strikeRangeFloor && spacing <= strikeRangeCeil)
									{
										leg2Indexes.Add(i);
									}
								}
						...
						if (gapList != null)
						{
							DateTime leg1Date = leg1.Expiration.Date;
							leg2Indexes.RemoveAll(i =>
							{
								DateTime leg2Date = optionsInGroup[i].Expiration.Date;
								if (!expDateToSortedIndex.TryGetValue(leg1Date, out int idx1) ||
									!expDateToSortedIndex.TryGetValue(leg2Date, out int idx2))
									return true;
								int gap = Math.Abs(idx2 - idx1) - 1;
								return !gapList.Contains(gap);
							});
						}
						...
						if (gapRangeEnabled)
						{
							...
							leg2Indexes.RemoveAll(i =>
							{
								...
								if (!expDateToSortedIndex.TryGetValue(leg1Date, out int idx1) ||
									!expDateToSortedIndex.TryGetValue(leg2Date, out int idx2))
									return true;
								int count = Math.Abs(idx2 - idx1) + 1;
								return count <= 0 || count - 1 < gapRangeFloor || count - 1 > gapRangeCeil;
							});
						}

						for (int li = 0; li < leg2Indexes.Count; li++)
						{
							token.ThrowIfCancellationRequested();
							Option? leg2 = optionsInGroup[leg2Indexes[li]];
							...
							if (Math.Abs(leg1.Strike - leg2.Strike) < STRIKE_TOLERANCE)
							{
								continue;
							}
