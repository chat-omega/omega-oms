...
{
    public partial class SelfTradeWarningUpdateTopic : ISelfTradeWarningUpdateTopic
    {
        ...
        private readonly ConcurrentDictionary<ulong, ISelfTradeModel> _indexToOrderUpdateMap;
        ...
        public SelfTradeWarningUpdateTopic(ILogger<SpreadRiskUpdateTopic> logger)
        {
            _logger = logger;
            _indexToOrderUpdateMap = new ConcurrentDictionary<ulong, ISelfTradeModel>();
        ...
        public bool TryEncodeAndSend(ulong index, IMessageSender sender, IEncodeBufferContext encodeContext, out ulong nextIndex)
        {
            ...
            var ctx = (SbeEncodeBufferContext)encodeContext;
                ...
                foreach (ISelfTradeModel update in updates)
                {
                    int written = ctx.Encoder.EncodeSelfTradeWarningModel(ctx.DirectBuffer, 0, update);
        ...
        public void Add(ISelfTradeModel model)
        {
            lock (_indexLock)
            {
        ...
        private HashSet<ISelfTradeModel> GetUpdatesSince(ulong index, out ulong nextIndex)
        {
            HashSet<ISelfTradeModel> newChanges = new HashSet<ISelfTradeModel>();
