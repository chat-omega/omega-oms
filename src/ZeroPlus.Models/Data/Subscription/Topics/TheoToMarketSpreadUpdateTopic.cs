...
public class TheoToMarketSpreadUpdateTopic : ITheoToMarketSpreadUpdateTopic
{
    ...
    public TheoToMarketSpread UpdateModel { get; }
    public ulong Index { get => _index; set => _index = value; }

    public TheoToMarketSpreadUpdateTopic(ILogger<DerivedValueUpdateTopic> logger)
    {
        _logger = logger;
        _index = 0;

        UpdateModel = new TheoToMarketSpread();
        Id = Guid.NewGuid();
        Compressed = false;
    ...
    public bool TryEncodeAndSend(ulong index, IMessageSender sender, IEncodeBufferContext encodeContext, out ulong nextIndex)
    {
        try
        {
            ...
            int written = ctx.Encoder.EncodeTheoToMarketSpreadUpdate(ctx.DirectBuffer, 0, UpdateModel);
