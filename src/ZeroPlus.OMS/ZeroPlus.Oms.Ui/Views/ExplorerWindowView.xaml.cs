...
namespace ZeroPlus.Oms.Ui.Views
{
    ...
    /// </summary>
    public partial class ExplorerWindowView : ModuleWindow, IModuleView
    {
        public static OmsCore OmsCore => ServiceLocator.GetService<OmsCore>();
        public ExplorerWindowView(IModuleFactory moduleFactory, string uid = null) : base(Module.ExplorerWindow, uid, moduleFactory)
        {
            InitializeComponent();
            Module = Module.ExplorerWindow;
        ...
        {
            ExplorerViewConfig viewConfig = new()
            {
                WindowSetting = new WindowSetting(this, isDefault).SerializeToJson(),
            };
            if (DataContext is ExplorerWindowViewModel viewModel)
            {
        ...
        {
            ExplorerViewConfig viewConfig = await ModuleConfigBase.DeserializeAsync<ExplorerViewConfig>(configJson);
            if (viewConfig != null)
            {
        ...
        private void TableView_DropRecord(object sender, DropRecordEventArgs e)
        {
            try
            {
                ExplorerWindowViewModel viewModel = ViewModel as ExplorerWindowViewModel;
