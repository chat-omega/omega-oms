...
{
    /// <summary>
    /// Interaction logic for DualOrderTicketView.xaml
    /// </summary>
    public partial class DualOrderTicketView : ThemedWindow
    {
        ...
        public DualOrderTicketView() : this(Guid.NewGuid().ToString())
        {
        }

        public DualOrderTicketView(string uid)
        {
            Uid = uid;
            lock (_initLock)
            {
            ...
            ((OrderTicket)DataContext).TicketStyle = Oms.Enums.OrderTicketStyle.Dual;
            Module = Module.DualOrderTicketLayout;
