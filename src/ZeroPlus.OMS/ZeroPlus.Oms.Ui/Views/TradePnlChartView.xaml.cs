...
{
    /// <summary>
    /// Interaction logic for TradePnlChartView.xaml
    /// </summary>
    public partial class TradePnlChartView : ThemedWindow
    {
        ...
        public TradePnlChartView() : this(Guid.NewGuid().ToString())
        {
        }

        public TradePnlChartView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Name = nameof(TradePnlChartView);
            Closed += View_Closed;
            Loaded += RestoreLayout;
            Module = Module.TradeChartLayout;
        ...
        {
            TradePnlChartViewModel dataContext = (TradePnlChartViewModel)DataContext;
            dataContext.Dispose();
        }
        ...
        private async void RestoreFromConfigSaveId(int configSaveId)
        {
            try
            {
                ConfigSave configSave = await (DataContext as TradePnlChartViewModel).OmsCore.GatewayClient.RequestConfigDataAsync(configSaveId);
                RestoreFromConfigSave(configSave);
            }
            catch (Exception ex)
            {
        ...
        private void SetTitleFromConfigSave(ConfigSave configSave)
        {
            ...
            {
                if (DataContext is TradePnlChartViewModel viewModel)
                {
        ...
        private string GetConfigAsJson(bool isDefault = false)
        {
            ...
            {
                [nameof(TradePnlChartViewModel.ChartModuleConfig)] = datacontext.GetConfigJson(),
        ...
        internal async Task LoadConfigFromJsonAsync(string configJson)
        {
            try
            {
                ...
                TradePnlChartViewModel datacontext = (TradePnlChartViewModel)DataContext;
                _ = datacontext.LoadConfigFromJsonAsync(configDictionary[nameof(TradePnlChartViewModel.ChartModuleConfig)]);
