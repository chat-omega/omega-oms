...
{
    /// <summary>
    /// Interaction logic for HedgeHouseView.xaml
    /// </summary>
    public partial class HedgeHouseView : ThemedWindow
    {
        private const string MODULE_NAME = "Hedge House";
        ...
        public HedgeHouseView() : this(Guid.NewGuid().ToString())
        {
        }

        public HedgeHouseView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Name = nameof(HedgeHouseView);
            ...
            Module = Module.HedgeHouseLayout;
        ...
        {
            HedgeHouseViewModel viewModel = (HedgeHouseViewModel)DataContext;
            viewModel.Dispose();
        }
        ...
        public void AddColumn(CustomColumnTemplateModel colTemplate)
        {
            ...
            if (HedgeHouseMainGrid.Columns.Any(x => x.FieldName == column.FieldName))
            {
                HedgeHouseMainGrid.Columns.First(x => x.FieldName == column.FieldName).Visible = true;
            }
            else
            {
                HedgeHouseMainGrid.Columns.Add(column);
            }
        }
        ...
        public List<CustomColumnTemplateModel> GetExpressionEditors()
        {
            ...
            foreach (GridColumn column in HedgeHouseMainGrid.Columns.Where(x => x.AllowUnboundExpressionEditor))
            {
        ...
        {
            HedgeHouseMainGrid.FilterCriteria = null;
            HedgeHouseMainGrid.FilterString = "";
        }

        private void ClearSortingClick(object sender, RoutedEventArgs e)
        {
            HedgeHouseMainGrid.ClearSorting();
        }

        private void ClearGrouoping()
        {
            foreach (GridColumn col in HedgeHouseMainGrid.Columns)
            {
                HedgeHouseMainGrid.UngroupBy(col);
            }
        }
        ...
        private void OnZoomMouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                HedgeHouseMainGridScale.Value += (e.Delta > 0) ? 0.1 : -0.1;
            }
        }
        ...
        private void SetTitleFromConfigSave(ConfigSave configSave)
        {
            ...
            {
                if (DataContext is HedgeHouseViewModel viewModel)
                {
        ...
        private string GetConfigAsJson(bool isDefault = false)
        {
            ...
            {
                [nameof(HedgeHouseMainGrid)] = LayoutHelper.GetLayoutAsString(HedgeHouseMainGrid),
        ...
        internal async Task LoadConfigFromJsonAsync(string configJson)
        {
            try
            {
                ...
                if (configDictionary.ContainsKey(nameof(HedgeHouseMainGrid)))
                {
                    LayoutHelper.RestoreLayoutFromString(configDictionary[nameof(HedgeHouseMainGrid)], HedgeHouseMainGrid);
                }
                ...
                if (configDictionary.ContainsKey(nameof(GridFieldNameToConfigMap)))
                {
                    if (configDictionary.TryGetValue(nameof(GridFieldNameToConfigMap), out string fieldMapConfig))
                    {
                        ...
                        tableCustomizationViewModel.Load(HedgeHouseMainGrid, GridFieldNameToConfigMap);
                    }
                }
            }
            catch (Exception ex)
