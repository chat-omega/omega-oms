...
{
    /// <summary>
    /// Interaction logic for PairTraderView.xaml
    /// </summary>
    public partial class PairTraderView : ThemedWindow
    {
        private const string MODULE_NAME = "Pairs Trader";
        ...
        public PairTraderView() : this(Guid.NewGuid().ToString()) { }

        public PairTraderView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Name = nameof(PairTraderView);
            OmsCore.SaveWorkspaceRequestEvent += OnSaveLayoutRequest;
            Closing += PairTraderView_Closing;
            Loaded += RestoreLayout;
            Loaded += SetupMacdContext;
            Module = Module.PairTrader;
        ...
        private void SetupMacdContext(object sender, RoutedEventArgs e)
        {
            if (DataContext is PairTraderViewModel viewModel)
            {
                viewModel.MacdContext = MacdControl.DataContext as MacdContext;
                viewModel.SetupMacdAutoTrading();
            }
            Loaded -= SetupMacdContext;
        }

        private void PairTraderView_Closing(object sender, CancelEventArgs e)
        {
            if (DataContext is PairTraderViewModel viewModel)
            {
            ...
            {
                _log.Error(nameof(PairTraderView_Closing), " view model not found.");
            }
        }
        ...
        private void RestoreLayout(object sender, RoutedEventArgs e)
        {
            ...
            PairTraderViewModel dataContext = (PairTraderViewModel)DataContext;
        ...
        private string GetConfigAsJson(bool isDefault = false)
        {
            ...
            Dictionary<string, string> configDictionary = new()
            {
                ...
                [nameof(PairTraderViewModel)] = dataContext.GetConfigAsJson(),
        ...
        internal async Task LoadConfigFromJsonAsync(string configJson)
        {
            try
            {
                ...
                if (configDictionary.TryGetValue(nameof(PairTraderViewModel), out string config))
                {
                    PairTraderViewModel dataContext = (PairTraderViewModel)DataContext;
                    dataContext.LoadConfigFromJson(config);
                }
                ...
                if (configDictionary.TryGetValue(nameof(OrdersGrid), out string positionGridLayout))
                {
                    LayoutHelper.RestoreLayoutFromString(positionGridLayout, OrdersGrid);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            foreach (GridColumn col in OrdersGrid.Columns)
            {
                OrdersGrid.UngroupBy(col);
            }
        }
        ...
        private void ExportToExcel(object sender, ItemClickEventArgs e)
        {
            try
            {
                TableView tableView = OrdersTableView;
                if (tableView != null)
                {
                    ISaveFileDialogService saveFileDialogService = ((PairTraderViewModel)DataContext).SaveFileDialogService;
                    saveFileDialogService.DefaultExt = "xlsx";
                    saveFileDialogService.DefaultFileName = $"Pairs Order Book Export - {DateTime.Now:MM-dd-yyyy hh.mm}";
        ...
        public List<CustomColumnTemplateModel> GetExpressionEditors()
        {
            ...
            foreach (GridColumn column in OrdersGrid.Columns.Where(x => x.AllowUnboundExpressionEditor))
            {
        ...
        public void AddColumn(CustomColumnTemplateModel colTemplate)
        {
            ...
            if (OrdersGrid.Columns.Any(x => x.FieldName == column.FieldName))
            {
                OrdersGrid.Columns.First(x => x.FieldName == column.FieldName).Visible = true;
            }
            else
            {
        ...
        {
            PairTraderView pairTraderView = new();
            pairTraderView.Loaded += (s, e) => _ = pairTraderView.LoadConfigFromJsonAsync(GetConfigAsJson());
            pairTraderView.Show();
        }
        ...
        private void MacdControl_Loaded(object sender, RoutedEventArgs e)
        {
