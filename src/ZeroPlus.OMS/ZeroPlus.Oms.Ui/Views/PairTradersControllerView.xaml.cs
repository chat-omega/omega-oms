...
{
    /// <summary>
    /// Interaction logic for PairTradersControllerView.xaml
    /// </summary>
    public partial class PairTradersControllerView : ThemedWindow
    {
        private const string MODULE_NAME = "Pair Traders Manager";
        ...
        public PairTradersControllerView() : this(Guid.NewGuid().ToString())
        {
        }

        public PairTradersControllerView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Name = nameof(PairTradersControllerView);
            ...
            Module = Module.PairTradersControllerLayout;
        ...
        {
            PairTradersControllerViewModel viewModel = (PairTradersControllerViewModel)DataContext;
            viewModel.Dispose();
        }
        ...
        public void AddColumn(CustomColumnTemplateModel colTemplate)
        {
            ...
            if (ModelsMainGrid.Columns.Any(x => x.FieldName == column.FieldName))
            {
                ModelsMainGrid.Columns.First(x => x.FieldName == column.FieldName).Visible = true;
            }
            else
            {
                ModelsMainGrid.Columns.Add(column);
            }
        }
        ...
        public List<CustomColumnTemplateModel> GetExpressionEditors()
        {
            ...
            foreach (GridColumn column in ModelsMainGrid.Columns.Where(x => x.AllowUnboundExpressionEditor))
            {
        ...
        {
            ModelsMainGrid.FilterCriteria = null;
            ModelsMainGrid.FilterString = "";
        }

        private void ClearSortingClick(object sender, RoutedEventArgs e)
        {
            ModelsMainGrid.ClearSorting();
        }

        private void ClearGrouoping()
        {
            foreach (GridColumn col in ModelsMainGrid.Columns)
            {
                ModelsMainGrid.UngroupBy(col);
            }
        }
        ...
        private void OnZoomMouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                ModelsMainGridScale.Value += (e.Delta > 0) ? 0.1 : -0.1;
            }
        }
        ...
        private void SetTitleFromConfigSave(ConfigSave configSave)
        {
            ...
            {
                if (DataContext is PairTradersControllerViewModel viewModel)
                {
        ...
        private string GetConfigAsJson(bool isDefault = false)
        {
            ...
            {
                [nameof(ModelsMainGrid)] = LayoutHelper.GetLayoutAsString(ModelsMainGrid),
        ...
        internal async Task LoadConfigFromJsonAsync(string configJson)
        {
            try
            {
                ...
                if (configDictionary.ContainsKey(nameof(ModelsMainGrid)))
                {
                    LayoutHelper.RestoreLayoutFromString(configDictionary[nameof(ModelsMainGrid)], ModelsMainGrid);
                }
                ...
                if (configDictionary.ContainsKey(nameof(GridFieldNameToConfigMap)))
                {
                    if (configDictionary.TryGetValue(nameof(GridFieldNameToConfigMap), out string fieldMapConfig))
                    {
                        ...
                        tableCustomizationViewModel.Load(ModelsMainGrid, GridFieldNameToConfigMap);
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            PairTradersControllerView view = new();
            view.Show();
        }
    }
}