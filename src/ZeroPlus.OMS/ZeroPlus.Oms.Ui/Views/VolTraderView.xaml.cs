...
{
    /// <summary>
    /// Interaction logic for VolTraderView.xaml
    /// </summary>
    public partial class VolTraderView : ThemedWindow, IModuleView, ISupportCustomColumn
    {
        private const string MODULE_NAME = "Vol Trader";
        ...
        public VolTraderView() : this(Guid.NewGuid().ToString())
        {
        }

        public VolTraderView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Reset();
            Name = nameof(VolTraderView);
            OmsCore.SaveWorkspaceRequestEvent += OnSaveLayoutRequest;
            Closing += (s, e) => OmsCore.SaveWorkspaceRequestEvent -= OnSaveLayoutRequest;
            Closing += VolTraderView_Closing;
            Loaded += RestoreLayout;
            Module = Module.VolTraderLayout;
        ...
        private void VolTraderView_Closing(object sender, CancelEventArgs e)
        {
            VolTraderViewModel viewModel = (VolTraderViewModel)DataContext;
            e.Cancel = viewModel.Dispose();
        }
        ...
        public void AddColumn(CustomColumnTemplateModel colTemplate)
        {
            ...
            if (VolTraderGrid.Columns.Any(x => x.FieldName == column.FieldName))
            {
                VolTraderGrid.Columns.First(x => x.FieldName == column.FieldName).Visible = true;
            }
            else
            {
                VolTraderGrid.Columns.Add(column);
            }
        }
        ...
        public List<CustomColumnTemplateModel> GetExpressionEditors()
        {
            ...
            foreach (GridColumn column in VolTraderGrid.Columns.Where(x => x.AllowUnboundExpressionEditor))
            {
        ...
        {
            VolTraderGrid.FilterCriteria = null;
            VolTraderGrid.FilterString = "";
        }

        private void ClearSortingClick(object sender, System.Windows.RoutedEventArgs e)
        {
            VolTraderGrid.ClearSorting();
        }

        private void ClearGrouoping()
        {
            foreach (GridColumn col in VolTraderGrid.Columns)
            {
                VolTraderGrid.UngroupBy(col);
            }
        }
        ...
        private void OnZoomMouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                VolTraderGridScale.Value += (e.Delta > 0) ? 0.1 : -0.1;
            }
        }
            ...
            if (!_layoutRestored)
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(defaultExportPath) && File.Exists(defaultExportPath))
                    {
                        ...
                        ConfigSave configSave = JsonConvert.DeserializeObject<ConfigSave>(export);
                        RestoreFromConfigSave(configSave, isDefault: true);
                    }
                    else
                    {
                        RestoreFromConfigSave(ConfigSave);
        ...
        internal void RestoreFromConfigSave(ConfigSave configSave, bool isDefault = false)
        {
            if (configSave != null)
            {
                SetTitleFromConfigSave(configSave);
                ConfigSave = configSave;
                _ = LoadConfigFromJsonAsync(configSave.ConfigJson, isDefault);
            }
        }
        ...
        private void SetTitleFromConfigSave(ConfigSave configSave)
        {
            ...
            {
                if (DataContext is VolTraderViewModel viewModel)
                {
        ...
        {
            VolTraderViewModel viewModel = DataContext as VolTraderViewModel;

            Dictionary<string, string> configDictionary = new()
            {
                [nameof(VolTraderGrid)] = LayoutHelper.GetLayoutAsString(VolTraderGrid),
                ...
                [nameof(VolTraderConfig)] = viewModel.GetViewModelConfigSerialized(),
            };
        ...
        internal async Task LoadConfigFromJsonAsync(string configJson, bool isDefault)
        {
            try
            {
                _layoutRestored = true;
                ...
                LayoutHelper.RestoreLayoutFromString(configDictionary[nameof(VolTraderGrid)], VolTraderGrid);
                ...
                if (configDictionary.TryGetValue(nameof(GridFieldNameToConfigMap), out string fieldMapConfig))
                {
                    ...
                    tableCustomizationViewModel.Load(VolTraderGrid, GridFieldNameToConfigMap);
                }
                if (configDictionary.TryGetValue(nameof(VolTraderConfig), out string volTraderConfig))
                {
                    VolTraderViewModel viewModel = DataContext as VolTraderViewModel;
                    viewModel.LoadFromConfig(volTraderConfig, isDefault);
                }
            }
            catch (Exception ex)
            {
        ...
        private void ResubmitSideChanged(object sender, EditValueChangedEventArgs e)
        {
            System.Collections.IList selected = VolTraderGrid.SelectedItems;
                 ...
                 selected.Count > 1)
            {
                ...
                {
                    if (item is BasketTraderViewModel basket)
                    {
                        basket.ResubmitSide = (ResubmitSide)e.NewValue;
                    }
                }
            }
        }

        private void EdgeTypeChanged(object sender, EditValueChangedEventArgs e)
        {
            System.Collections.IList selected = VolTraderGrid.SelectedItems;
            if ((Keyboard.IsKeyDown(Key.LeftAlt) ||
                 Keyboard.IsKeyDown(Key.RightAlt)) &&
                selected.Count > 1)
            {
                foreach (object item in selected)
                {
                    if (item is BasketTraderViewModel basket)
                    {
                        basket.BasketSettings.EdgeType = (string)e.NewValue;
                    }
                }
            }
        }

        private void ResubmitIntervalChanged(object sender, EditValueChangedEventArgs e)
        {
            System.Collections.IList selected = VolTraderGrid.SelectedItems;
            if ((Keyboard.IsKeyDown(Key.LeftAlt) ||
                 Keyboard.IsKeyDown(Key.RightAlt)) &&
                selected.Count > 1)
            {
                foreach (object item in selected)
                {
                    if (item is BasketTraderViewModel basket)
                    {
                        basket.ResubmitIntervalSec = (int)e.NewValue;
                    }
                }
            }
        }

        private void BuyEdgeChanged(object sender, EditValueChangedEventArgs e)
        {
            System.Collections.IList selected = VolTraderGrid.SelectedItems;
            if ((Keyboard.IsKeyDown(Key.LeftAlt) ||
                 Keyboard.IsKeyDown(Key.RightAlt)) &&
                selected.Count > 1)
            {
                foreach (object item in selected)
                {
                    if (item is BasketTraderViewModel basket)
                    {
                        basket.BasketSettings.BuyEdge = (double)e.NewValue;
                    }
                }
            }
        }

        private void SellEdgeChanged(object sender, EditValueChangedEventArgs e)
        {
            System.Collections.IList selected = VolTraderGrid.SelectedItems;
            if ((Keyboard.IsKeyDown(Key.LeftAlt) ||
                 Keyboard.IsKeyDown(Key.RightAlt)) &&
                selected.Count > 1)
            {
                foreach (object item in selected)
                {
                    if (item is BasketTraderViewModel basket)
                    {
                        basket.BasketSettings.SellEdge = (double)e.NewValue;
                    }
                }
            }
        }
