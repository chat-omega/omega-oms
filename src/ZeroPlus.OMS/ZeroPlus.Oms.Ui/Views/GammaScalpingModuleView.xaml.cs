...
{
    /// <summary>
    /// Interaction logic for GammaScalpingModuleView.xaml
    /// </summary>
    public partial class GammaScalpingModuleView : ThemedWindow
    {
        private const string MODULE_NAME = "Gamma Scalp";
        ...
        public GammaScalpingModuleView() : this(Guid.NewGuid().ToString())
        {
        }

        public GammaScalpingModuleView(string uid)
        {
            Uid = uid;
            InitializeComponent();
            Name = nameof(GammaScalpingModuleView);
            ...
            Module = Module.GammaScalpLayout;
            ...
            GammaScalpingModuleViewModel viewModel = DataContext as GammaScalpingModuleViewModel;
            viewModel?.SetDispatcher(Dispatcher);
        }

        private void ViewClosed(object sender, EventArgs e)
        {
            GammaScalpingModuleViewModel viewModel = (GammaScalpingModuleViewModel)DataContext;
            viewModel?.Dispose();
        }
        ...
        public void AddColumn(CustomColumnTemplateModel colTemplate)
        {
            ...
            if (GammaScalpGrid.Columns.Any(x => x.FieldName == column.FieldName))
            {
                GammaScalpGrid.Columns.First(x => x.FieldName == column.FieldName).Visible = true;
            }
            else
            {
                GammaScalpGrid.Columns.Add(column);
            }
        }
        ...
        public List<CustomColumnTemplateModel> GetExpressionEditors()
        {
            ...
            foreach (GridColumn column in GammaScalpGrid.Columns.Where(x => x.AllowUnboundExpressionEditor))
            {
        ...
        {
            GammaScalpGrid.FilterCriteria = null;
            GammaScalpGrid.FilterString = "";
        }

        private void ClearSortingClick(object sender, System.Windows.RoutedEventArgs e)
        {
            GammaScalpGrid.ClearSorting();
        }

        private void ClearGrouoping()
        {
            foreach (GridColumn col in GammaScalpGrid.Columns)
            {
                GammaScalpGrid.UngroupBy(col);
            }
        }
        ...
        private void SetTitleFromConfigSave(ConfigSave configSave)
        {
            ...
            {
                if (DataContext is GammaScalpingModuleViewModel viewModel)
                {
        ...
        private void OnDropRecord(object sender, DropRecordEventArgs e)
        {
                ...
                if (e.Data.GetDataPresent(DataFormats.CommaSeparatedValue))
                {
                    ...
                    if (dragItem != null)
                    {
                        ...
                        if (items.Length > 0)
                        {
                            ...
                            if (!viewModel.UnderlyingLoaded || viewModel.Underlying == underlying)
                            {
                                viewModel.OrderTicket.LoadLegsFromTosAsync(symbol, null, true, true);
                                if (viewModel.Underlying != viewModel.OrderTicket.Underlying)
                                {
                                    viewModel.Underlying = viewModel.HedgeUnderlying = viewModel.OrderTicket.Underlying;
                                    viewModel.HedgeMultiplier = 1;
                                }
                            }
                        }
                    }
            ...
            {
                _log.Error(ex, nameof(OnDropRecord));
            }
        }
